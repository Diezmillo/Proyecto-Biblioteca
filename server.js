...
// API PRESTAR LIBRO
app.post("/api/prestamos/crear/:id", async (req, res) => {
    try {
        if (!req.session.user) {
            return res.status(401).json({
                success: false,
        ...
        await db.query(`
            ...
            (user_id, libro_id, fecha_prestamo, fecha_devolucion)
            VALUES (?, ?, CURDATE(), ?)
        `, [user, libro, fecha_devolucion]);
        await db.query(
        ...
        res.status(500).json({
            success: false,
            message: "Error al prestar libro"
        });
    }
});
// API DEVOLVER LIBRO
...
// API MIS PRESTAMOS
app.get("/api/mis-prestamos", async (req, res) => {
    try {
        if (!req.session.user) {
            return res.status(401).json({
                success: false,
